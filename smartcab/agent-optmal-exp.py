...
class LearningAgent(Agent):
    ...
    def choose_action(self, state):
        ...
        maxQ_pair={k:v for k,v in self.Q[state].items() if v==maxQ}
        maxQ_actions=maxQ_pair.keys()

        if self.learning and random.random()<self.epsilon:
            action = random.choice(self.valid_actions)
        elif self.learning and len(maxQ_actions)>1:
...
def run():
    ...
    sim.run(tolerance=0.00001,n_test=20)
